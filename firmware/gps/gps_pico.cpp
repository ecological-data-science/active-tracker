...
int64_t millis() { return to_ms_since_boot(get_absolute_time()); }

gps_pico::gps_pico(void) {
...
size_t gps_pico::getPacketCfgSpaceRemaining() {
...
uint16_t gps_pico::available() { return _i2cBus.available(); }
...
uint8_t gps_pico::writeBytes(uint8_t *data, uint8_t length) {
  return _i2cBus.writeBytes(data, length);
}
// For SPI, writing bytes will also read bytes simultaneously. Read data is
// returned in readData
uint8_t gps_pico::writeReadBytes(const uint8_t *data, uint8_t *readData,
                                     uint8_t length) {
  return _i2cBus.writeReadBytes(data, readData, length);
}
void gps_pico::startWriteReadByte() { _i2cBus.startWriteReadByte(); }
void gps_pico::writeReadByte(const uint8_t *data, uint8_t *readData) {
  _i2cBus.writeReadByte(data, readData);
}
void gps_pico::writeReadByte(const uint8_t data, uint8_t *readData) {
  _i2cBus.writeReadByte(data, readData);
}
void gps_pico::endWriteReadByte() { _i2cBus.endWriteReadByte(); }
// For Serial, attempt Serial.read
// For I2C, read from register 0xFF
// For SPI, read the byte while writing 0xFF
uint8_t gps_pico::readBytes(uint8_t *data, uint8_t length) {
  return _i2cBus.readBytes(data, length);
}

bool gps_pico::init(uint16_t maxWait, bool assumeSuccess) {
...
void gps_pico::setI2CTransactionSize(uint8_t transactionSize) {
...
uint8_t gps_pico::getI2CTransactionSize(void) {
  return (i2cTransactionSize);
}

// Returns true if I2C device ack's
bool gps_pico::isConnected(uint16_t maxWait) {
  if (!ping())
...
const char *gps_pico::statusString(sfe_ublox_status_e stat) {
  switch (stat) {
...
bool gps_pico::checkUblox(uint8_t requestedClass, uint8_t requestedID) {
...
// port
bool gps_pico::checkUbloxInternal(ubxPacket *incomingUBX,
...
bool gps_pico::checkUbloxI2C(ubxPacket *incomingUBX, uint8_t requestedClass,
                                 uint8_t requestedID) {
...
bool gps_pico::setPacketCfgPayloadSize(size_t payloadSize) {
  bool success = true;
...
void gps_pico::process(uint8_t incoming, ubxPacket *incomingUBX,
...
bool gps_pico::isNMEAHeaderValid() {
...
void gps_pico::processUBX(uint8_t incoming, ubxPacket *incomingUBX,
                              uint8_t requestedClass, uint8_t requestedID) {
...
void gps_pico::processUBXpacket(ubxPacket *msg) {
  switch (msg->cls) {
  case UBX_CLASS_NAV:
...
void gps_pico::calcChecksum(ubxPacket *msg) {
  msg->checksumA = 0;
  msg->checksumB = 0;
...
void gps_pico::addToChecksum(uint8_t incoming) {
...
// port
sfe_ublox_status_e gps_pico::sendCommand(ubxPacket *outgoingUBX,
                                             uint16_t maxWait,
                                             bool expectACKonly) {
  if (!lock())
...
sfe_ublox_status_e gps_pico::sendI2cCommand(ubxPacket *outgoingUBX) {
...
void gps_pico::printPacket(ubxPacket *packet, bool alwaysPrintPayload) {
...
//  very slowly)
sfe_ublox_status_e gps_pico::waitForACKResponse(ubxPacket *outgoingUBX,
                                                    uint8_t requestedClass,
                                                    uint8_t requestedID,
                                                    uint16_t maxTime) {
  outgoingUBX->valid =
...
//  or is currently being overwritten (remember that Serial data can arrive
//  very slowly)
sfe_ublox_status_e gps_pico::waitForNoACKResponse(ubxPacket *outgoingUBX,
                                                      uint8_t requestedClass,
                                                      uint8_t requestedID,
                                                      uint16_t maxTime) {
  outgoingUBX->valid =
...
bool gps_pico::setI2CAddress(uint8_t deviceAddress, uint8_t layer,
                                 uint16_t maxWait) {
...
bool gps_pico::setSerialRate(uint32_t baudrate, uint8_t uartPort,
...
bool gps_pico::setI2COutput(uint8_t comSettings, uint8_t layer,
                                uint16_t maxWait) {
...
bool gps_pico::setI2CInput(uint8_t comSettings, uint8_t layer,
                               uint16_t maxWait) {
  bool result = newCfgValset(layer);
...
void gps_pico::cfgRst(uint8_t *data, uint8_t len) {
...
bool gps_pico::factoryDefault(uint16_t maxWait) {
...
bool gps_pico::cfgCfg(uint8_t *data, uint8_t len, uint16_t maxWait) {
  packetCfg.cls = UBX_CLASS_CFG;
...
bool gps_pico::setDGNSSConfiguration(sfe_ublox_dgnss_mode_e dgnssMode,
                                         uint8_t layer, uint16_t maxWait) {
...
uint8_t gps_pico::getProtocolVersionHigh(uint16_t maxWait) {
  if (!prepareModuleInfo(maxWait))
    return 0;
  return (moduleSWVersion->protocolVersionHigh);
}
uint8_t gps_pico::getProtocolVersionLow(uint16_t maxWait) {
  if (!prepareModuleInfo(maxWait))
    return 0;
...
uint8_t gps_pico::getFirmwareVersionHigh(uint16_t maxWait) {
  if (!prepareModuleInfo(maxWait))
    return 0;
  return (moduleSWVersion->firmwareVersionHigh);
}
uint8_t gps_pico::getFirmwareVersionLow(uint16_t maxWait) {
  if (!prepareModuleInfo(maxWait))
    return 0;
...
const char *gps_pico::getFirmwareType(uint16_t maxWait) {
...
// Get the module name
const char *gps_pico::getModuleName(uint16_t maxWait) {
...
bool gps_pico::prepareModuleInfo(uint16_t maxWait) {
  if (moduleSWVersion == nullptr)
...
// with This is helpful when deciding if we should call the high-precision
// Lat/Long (HPPOSLLH) or the regular (POSLLH)
bool gps_pico::getProtocolVersion(uint16_t maxWait) // Old name - deprecated
{
  return getModuleInfo(maxWait);
}

bool gps_pico::getModuleInfo(uint16_t maxWait) {
  if (moduleSWVersion == nullptr)
    initModuleSWVersion(); // Check that RAM has been allocated for the SW
                           // version
  if (moduleSWVersion == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::initModuleSWVersion() {
...
// = 0.
bool gps_pico::powerOff(uint32_t durationInMs, uint16_t maxWait) {
...
bool gps_pico::powerOffWithInterrupt(uint32_t durationInMs,
...
bool gps_pico::setDynamicModel(dynModel newDynamicModel, uint8_t layer,
                                   uint16_t maxWait) {
...
uint8_t gps_pico::getDynamicModel(uint8_t layer, uint16_t maxWait) {
  uint8_t model;
...
uint32_t gps_pico::getEnableGNSSConfigKey(sfe_ublox_gnss_ids_e id) {
...
bool gps_pico::enableGNSS(bool enable, sfe_ublox_gnss_ids_e id,
                              uint8_t layer, uint16_t maxWait) {
...
bool gps_pico::isGNSSenabled(sfe_ublox_gnss_ids_e id, bool *enabled,
                                 uint8_t layer, uint16_t maxWait) {
  uint32_t key = getEnableGNSSConfigKey(id);
  return (getVal8(key, (uint8_t *)enabled, layer, maxWait));
}
bool gps_pico::isGNSSenabled(sfe_ublox_gnss_ids_e id, uint8_t layer,
...
uint8_t
gps_pico::getAckAiding(uint8_t layer,
...
bool gps_pico::setAckAiding(uint8_t ackAiding, uint8_t layer,
...
uint8_t gps_pico::getAopCfg(uint8_t layer, uint16_t maxWait) {
  uint8_t enabled;
...
bool gps_pico::setAopCfg(uint8_t aopCfg, uint16_t aopOrbMaxErr,
                             uint8_t layer, uint16_t maxWait) {
  bool result = newCfgValset(layer);
...
// (version 2)
bool gps_pico::getUniqueChipId(UBX_SEC_UNIQID_data_t *data,
                                   uint16_t maxWait) {
...
sfe_ublox_status_e gps_pico::getVal(uint32_t key, uint8_t layer,
                                        uint16_t maxWait) {
  packetCfg.cls = UBX_CLASS_CFG;
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::getVal8(uint32_t key, uint8_t *val, uint8_t layer,
                           uint16_t maxWait) {
...
uint8_t gps_pico::getVal8(uint32_t key, uint8_t layer,
...
bool gps_pico::getValSigned8(uint32_t key, int8_t *val, uint8_t layer,
                                 uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) == SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
bool gps_pico::getVal16(uint32_t key, uint16_t *val, uint8_t layer,
                            uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) == SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
uint16_t gps_pico::getVal16(uint32_t key, uint8_t layer,
                                uint16_t maxWait) // Unsafe overload
{
  uint16_t result = 0;
...
bool gps_pico::getValSigned16(uint32_t key, int16_t *val, uint8_t layer,
                                  uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) == SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
bool gps_pico::getVal32(uint32_t key, uint32_t *val, uint8_t layer,
                            uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) == SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
uint32_t gps_pico::getVal32(uint32_t key, uint8_t layer,
                                uint16_t maxWait) // Unsafe overload
{
  uint32_t result = 0;
...
bool gps_pico::getValSigned32(uint32_t key, int32_t *val, uint8_t layer,
                                  uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) == SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
bool gps_pico::getVal64(uint32_t key, uint64_t *val, uint8_t layer,
                            uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) != SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
uint64_t gps_pico::getVal64(uint32_t key, uint8_t layer,
                                uint16_t maxWait) // Unsafe overload
{
  uint64_t result = 0;
...
bool gps_pico::getValSigned64(uint32_t key, int64_t *val, uint8_t layer,
                                  uint16_t maxWait) {
  bool result = getVal(key, layer, maxWait) != SFE_UBLOX_STATUS_DATA_RECEIVED;
  if (result)
...
bool gps_pico::getValFloat(uint32_t key, float *val, uint8_t layer,
                               uint16_t maxWait) {
  if (sizeof(float) != 4)
    return false;
...
bool gps_pico::getValDouble(uint32_t key, double *val, uint8_t layer,
                                uint16_t maxWait) {
  if (sizeof(double) != 8)
    return false;
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::setValN(uint32_t key, uint8_t *value, uint8_t N,
                           uint8_t layer, uint16_t maxWait) {
  packetCfg.cls = UBX_CLASS_CFG;
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::setVal8(uint32_t key, uint8_t value, uint8_t layer,
                           uint16_t maxWait) {
  uint8_t val[1] = {value};
  return (setValN(key, val, 1, layer, maxWait));
}
bool gps_pico::setValSigned8(uint32_t key, int8_t value, uint8_t layer,
                                 uint16_t maxWait) {
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::setVal16(uint32_t key, uint16_t value, uint8_t layer,
                            uint16_t maxWait) {
  uint8_t val[2] = {(uint8_t)(value >> 0), (uint8_t)(value >> 8)};
  return (setValN(key, val, 2, layer, maxWait));
}
bool gps_pico::setValSigned16(uint32_t key, int16_t value, uint8_t layer,
                                  uint16_t maxWait) {
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::setVal32(uint32_t key, uint32_t value, uint8_t layer,
                            uint16_t maxWait) {
  uint8_t val[4] = {(uint8_t)(value >> 0), (uint8_t)(value >> 8),
                    (uint8_t)(value >> 16), (uint8_t)(value >> 24)};
  return (setValN(key, val, 4, layer, maxWait));
}
bool gps_pico::setValSigned32(uint32_t key, int32_t value, uint8_t layer,
                                  uint16_t maxWait) {
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::setVal64(uint32_t key, uint64_t value, uint8_t layer,
                            uint16_t maxWait) {
  uint8_t val[8];
...
bool gps_pico::setValSigned64(uint32_t key, int64_t value, uint8_t layer,
                                  uint16_t maxWait) {
...
bool gps_pico::setValFloat(uint32_t key, float value, uint8_t layer,
                               uint16_t maxWait) {
  if (sizeof(float) != 4) {
    return false;
  }
...
bool gps_pico::setValDouble(uint32_t key, double value, uint8_t layer,
                                uint16_t maxWait) {
  if (sizeof(double) != 8) {
    return false;
  }
...
// Configuration of modern u-blox modules is now done via
// getVal/setVal/delVal, ie protocol v27 and above found on ZED-F9P
bool gps_pico::newCfgValset(uint8_t layer) {
  packetCfg.cls = UBX_CLASS_CFG;
  packetCfg.id = UBX_CFG_VALSET;
...
bool gps_pico::addCfgValsetN(uint32_t key, uint8_t *value, uint8_t N) {
...
bool gps_pico::addCfgValset64(uint32_t key, uint64_t value) {
  uint8_t val[8];
...
bool gps_pico::addCfgValset32(uint32_t key, uint32_t value) {
  uint8_t val[4] = {(uint8_t)(value >> 0), (uint8_t)(value >> 8),
                    (uint8_t)(value >> 16), (uint8_t)(value >> 24)};
...
bool gps_pico::addCfgValset16(uint32_t key, uint16_t value) {
  uint8_t val[2] = {(uint8_t)(value >> 0), (uint8_t)(value >> 8)};
...
bool gps_pico::addCfgValset8(uint32_t key, uint8_t value) {
  uint8_t val[1] = {value};
...
bool gps_pico::addCfgValsetFloat(uint32_t key, float value) {
  if (sizeof(float) != 4) {
    return false;
  }
...
bool gps_pico::addCfgValsetDouble(uint32_t key, double value) {
  if (sizeof(double) != 8) {
    return false;
  }
...
bool gps_pico::sendCfgValset(uint16_t maxWait) {
  if (_numCfgKeys == 0)
...
uint8_t gps_pico::getCfgValsetLen() { return _numCfgKeys; }

// Return the number of free bytes remaining in packetCfgPayload
size_t gps_pico::getCfgValsetSpaceRemaining() {
  return getPacketCfgSpaceRemaining();
}

// Deprecated - only included for backward-compatibility. Use newCfgValset and
// sendCfgValset
bool gps_pico::newCfgValset8(uint32_t key, uint8_t value, uint8_t layer) {
  bool result = newCfgValset(layer);
  result &= addCfgValset8(key, value);
  return result;
}
bool gps_pico::newCfgValset16(uint32_t key, uint16_t value, uint8_t layer) {
  bool result = newCfgValset(layer);
  result &= addCfgValset16(key, value);
  return result;
}
bool gps_pico::newCfgValset32(uint32_t key, uint32_t value, uint8_t layer) {
  bool result = newCfgValset(layer);
  result &= addCfgValset32(key, value);
  return result;
}
bool gps_pico::newCfgValset64(uint32_t key, uint64_t value, uint8_t layer) {
  bool result = newCfgValset(layer);
  result &= addCfgValset64(key, value);
  return result;
}
bool gps_pico::sendCfgValset8(uint32_t key, uint8_t value,
                                  uint16_t maxWait) {
  bool result = addCfgValset8(key, value);
  result &= sendCfgValset(maxWait);
  return result;
}
bool gps_pico::sendCfgValset16(uint32_t key, uint16_t value,
                                   uint16_t maxWait) {
  bool result = addCfgValset16(key, value);
  result &= sendCfgValset(maxWait);
  return result;
}
bool gps_pico::sendCfgValset32(uint32_t key, uint32_t value,
                                   uint16_t maxWait) {
  bool result = addCfgValset32(key, value);
  result &= sendCfgValset(maxWait);
  return result;
}
bool gps_pico::sendCfgValset64(uint32_t key, uint64_t value,
                                   uint16_t maxWait) {
...
uint8_t gps_pico::getCfgValueSizeBytes(const uint32_t key) {
...
// variables
bool gps_pico::getPVT(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
...
bool gps_pico::initPacketUBXNAVPVT() {
...
bool gps_pico::setNavigationFrequency(uint8_t navFreq, uint8_t layer,
                                          uint16_t maxWait) {
...
bool gps_pico::getNavigationFrequency(uint8_t *navFreq, uint8_t layer,
                                          uint16_t maxWait) {
...
uint8_t
gps_pico::getNavigationFrequency(uint8_t layer,
                                     uint16_t maxWait) // Unsafe overload...
{
  uint8_t navFreq = 0;
...
// defines the rate
bool gps_pico::setMeasurementRate(uint16_t rate, uint8_t layer,
                                      uint16_t maxWait) {
...
// defines the rate
bool gps_pico::getMeasurementRate(uint16_t *measRate, uint8_t layer,
                                      uint16_t maxWait) {
...
uint16_t
gps_pico::getMeasurementRate(uint8_t layer,
                                 uint16_t maxWait) // Unsafe overload...
{
  uint16_t measurementRate;
...
bool gps_pico::setNavigationRate(uint16_t rate, uint8_t layer,
                                     uint16_t maxWait) {
...
bool gps_pico::getNavigationRate(uint16_t *navRate, uint8_t layer,
                                     uint16_t maxWait) {
...
uint16_t gps_pico::getNavigationRate(uint8_t layer,
                                         uint16_t maxWait) // Unsafe overload...
{
  uint16_t navigationRate;
...
uint32_t gps_pico::getTimeOfWeek(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint16_t gps_pico::getYear(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getMonth(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
// Get the current day
uint8_t gps_pico::getDay(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getHour(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getMinute(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getSecond(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint16_t gps_pico::getMillisecond(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
int32_t gps_pico::getNanosecond(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint32_t gps_pico::getUnixEpoch(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint32_t gps_pico::getUnixEpoch(uint32_t &microsecond, uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getDateValid(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getTimeValid(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getTimeFullyResolved(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getConfirmedDate(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getConfirmedTime(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getFixType(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getGnssFixOk(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getDiffSoln(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
bool gps_pico::getHeadVehValid(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getCarrierSolutionType(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint8_t gps_pico::getSIV(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
int32_t gps_pico::getLongitude(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
// Returns a long representing the number of degrees *10^-7
int32_t gps_pico::getLatitude(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
int32_t gps_pico::getAltitude(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint16_t gps_pico::getPDOP(uint16_t maxWait) {
  if (packetUBXNAVPVT == nullptr)
    initPacketUBXNAVPVT(); // Check that RAM has been allocated for the PVT
                           // data
  if (packetUBXNAVPVT == nullptr) // Bail if the RAM allocation failed
...
uint64_t gps_pico::extractLongLong(ubxPacket *msg, uint16_t spotToStart) {
  uint64_t val = 0;
  for (uint8_t i = 0; i < 8; i++)
...
int64_t gps_pico::extractSignedLongLong(ubxPacket *msg,
...
uint32_t gps_pico::extractLong(ubxPacket *msg, uint16_t spotToStart) {
  uint32_t val = 0;
  for (uint8_t i = 0; i < 4; i++)
...
// int32_t correctly...
int32_t gps_pico::extractSignedLong(ubxPacket *msg, uint16_t spotToStart) {
  unsignedSigned32 converter;
...
uint16_t gps_pico::extractInt(ubxPacket *msg, uint16_t spotToStart) {
...
// int16_t correctly...
int16_t gps_pico::extractSignedInt(ubxPacket *msg, uint16_t spotToStart) {
  unsignedSigned16 converter;
...
uint8_t gps_pico::extractByte(ubxPacket *msg, uint16_t spotToStart) {
  return (msg->payload[spotToStart]);
}

// Given a spot, extract a signed 8-bit value from the payload
int8_t gps_pico::extractSignedChar(ubxPacket *msg, uint16_t spotToStart) {
  unsignedSigned8 converter;
...
float gps_pico::extractFloat(ubxPacket *msg, uint16_t spotToStart) {
  unsigned32float converter;
  converter.unsigned32 = extractLong(msg, spotToStart);
...
double gps_pico::extractDouble(ubxPacket *msg, uint16_t spotToStart) {
  unsigned64double converter;
...
// bit start
uint64_t gps_pico::extractUnsignedBits(uint8_t *ptr, uint16_t start,
                                           uint16_t width) {
  uint64_t result = 0;
  uint16_t count = 0;
...
// start
int64_t gps_pico::extractSignedBits(uint8_t *ptr, uint16_t start,
                                        uint16_t width) {
